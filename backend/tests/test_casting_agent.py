...
# SHARE's fire-and-forget delivery-result watcher (Production fix,
# 2026-09-08) — no real WhatsApp worker runs during tests, so jobs stay
# "pending" forever; without this override every SHARE-approval test
# would leave a live 10-minute background polling task running for the
# rest of this file's shared event loop. Set BEFORE the agents modules
...
async def _use_test_config(group_name: str, agent_id: str = AGENT_ID):
    """Point an agent's routing at a throwaway group name for this test,
    saving whatever config already existed so it can be restored — this
    is a shared local dev DB, not a disposable test database.

    agent_id defaults to casting-agent (every pre-existing call site is
    unaffected); SHARE ownership/routing tests (Production fix,
    2026-09-05 — standalone SHARE moved to whatsapp-campaign-agent) pass
    agent_id="whatsapp-campaign-agent" explicitly instead."""
...
# now runs in the Talentgram WhatsApp Agent group; every standalone-SHARE
# test below configures THIS agent instead of casting-agent's. Thin
# wrappers (not a new helper shape) so every call site stays a one-line
# diff from the pre-routing-fix version.
SHARE_AGENT_ID = "whatsapp-campaign-agent"


async def _use_share_test_config(group_name: str):
    return await _use_test_config(group_name, agent_id=SHARE_AGENT_ID)


async def _restore_share_config(original):
...
async def test_share_legacy_hyphen_syntax_redirects_not_parsed():
    """Production Readiness (Part 17, 2026-09-03) — the OLD "share -
    Project - Talent" structured form is no longer silently parsed as a
    second competing grammar; it's redirected to the new canonical
    format, with a concrete example, and nothing is sent. The underlying
    send mechanism itself is completely unaffected (proven by every OTHER
    SHARE test in this file using the natural-language form)."""
    group = f"Test Casting {uuid.uuid4().hex[:6]}"
    original = await _use_share_test_config(group)
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    ...
    try:
        ...
        assert r.handled, r.reply
        assert "Please use the new SHARE format." in r.reply, r.reply
        assert "Share casting call for Hinge with Anusha Sharma" in r.reply, r.reply
...
async def test_share_near_miss_leading_filler_gives_help_guidance():
    """A near-miss SHARE with a leading filler word ("pls share...")
    defeats the normal leading-trigger match entirely — before the
    2026-09-08 fix this was silently dropped (handled=False, no reply
    at all). Must now give the SAME instructional guidance HELP's own
    SHARE examples show, never silence, never a guess/execution."""
    group = f"Test Casting {uuid.uuid4().hex[:6]}"
    original = await _use_share_test_config(group)
    phone = _phone()
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text="pls share casting call with anusha for hinge",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
        assert "I understood this as a SHARE command" in r.reply, r.reply
        assert "Share casting call for Hinge with Anusha Sharma" in r.reply, r.reply
...
async def test_share_near_miss_sending_with_content_word_gives_guidance():
    """"sending casting call..." (never a recognized trigger word or a
    one-edit-away typo of one) is still recognized as an ATTEMPTED
    SHARE because "casting call" is an explicit content signal — gets
    ...
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text="sending casting call to anusha for hinge",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
        assert "I understood this as a SHARE command" in r.reply, r.reply
...
async def test_share_ordinary_chatter_still_silently_ignored():
    """The near-miss heuristic must stay narrow — ordinary group
    chatter that happens to contain "sending"/"forward" with no SHARE
    content signal, or nothing share-related at all, is still silently
    ignored exactly as before this feature existed (never spammed with
    unwanted guidance)."""
    group = f"Test Casting {uuid.uuid4().hex[:6]}"
    original = await _use_share_test_config(group)
    phone = _phone()
    try:
        for text in ("sending the car to the shoot location", "good morning everyone", "the shoot is at 9am tomorrow"):
            r = await handle_inbound_message(
                group_name=group, sender_phone=phone, text=text,
                sender_name="Raj", sender_is_group_member=True,
            )
            assert r.handled is False, (text, r.reply)
    finally:
        await _cleanup(phone)
        await _restore_share_config(original)


async def test_share_missing_recipient_gives_guided_error():
    """Missing recipient entirely (no "to"/"with" clause at all) asks
    instead of guessing/erroring unhelpfully (Part 13)."""
    group = f"Test Casting {uuid.uuid4().hex[:6]}"
    original = await _use_share_test_config(group)
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    project_id = await _seed_project(brand_name=f"ShareNoRecipProj {tag}")
    label = (await db.projects.find_one({"id": project_id}))["brand_name"]
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f"share casting call for {label}",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
        assert "SHARE needs a talent or pipeline stage." in r.reply, r.reply
        assert "Share casting call for Hinge with Anusha Sharma" in r.reply, r.reply
    finally:
        await _cleanup(phone, project_ids=[project_id])
        await _restore_share_config(original)
...
async def test_share_custom_message_with_internal_comma_preserved():
    """Part 3's own exact worked example: a comma INSIDE the quoted custom
    message must never split recipients — only the comma OUTSIDE the
    quotes (between the two talent names) does. Two recipients, one
    ...
    tag = uuid.uuid4().hex[:6]
    project_id = await _seed_project_with_details(
        f"ShareCustomProj {tag}", shoot_dates="5 May 2028", budget="Rs 4,000/day",
    )
    t1 = await _seed_talent(f"ShareCustomT1 {tag}", phone="917000700040")
    t2 = await _seed_talent(f"ShareCustomT2 {tag}", phone="917000700041")
    await _seed_pipeline_row(project_id, t1, "ask_to_test")
    await _seed_pipeline_row(project_id, t2, "shortlisted")
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=(
                'Share custom message "Hi, please confirm your availability, '
                f'location and budget." with ShareCustomT1 {tag}, ShareCustomT2 {tag}'
            ),
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
        ...
        assert len(jobs) == 2
        for j in jobs:
            assert "Hi, please confirm your availability, location and budget." in j["message_body"]
    finally:
        await _cleanup_jobs_for_talents([t1, t2])
        await _cleanup(phone, project_ids=[project_id], talent_ids=[t1, t2])
        await _restore_share_config(original)
...
async def test_share_custom_message_to_pipeline_stage():
    """Part 4: a custom message can target a pipeline STAGE ("... to
    Follow Up for X"), reaching only talents currently in that stage —
    reusing the same stage-target machinery a saved-template SHARE uses,
    never a second/duplicate mechanism."""
    group = f"Test Casting {uuid.uuid4().hex[:6]}"
    original = await _use_share_test_config(group)
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    project_id = await _seed_project_with_details(
        f"ShareCustomStageProj {tag}", shoot_dates="6 Jun 2028", budget="Rs 5,000/day",
    )
    t1 = await _seed_talent(f"ShareCustomStageT1 {tag}", phone="917000700050")
    t2 = await _seed_talent(f"ShareCustomStageT2 {tag}", phone="917000700051")
    await _seed_pipeline_row(project_id, t1, "follow_up")
    await _seed_pipeline_row(project_id, t2, "shortlisted")  # different stage — must NOT receive it
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f'Share custom message "Quick update on your audition." to Follow Up for ShareCustomStageProj {tag} and confirm',
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
        assert "queued" in r.reply, r.reply
        ...
        assert "Quick update on your audition." in jobs[0]["message_body"]
    finally:
        await _cleanup_jobs_for_talents([t1, t2])
        await _cleanup(phone, project_ids=[project_id], talent_ids=[t1, t2])
        await _restore_share_config(original)
...
async def test_share_no_project_infers_single_ongoing_pipeline():
    """Part 1's own canonical example ("Share Casting Call with Anusha
    Sharma") never names a project — when the talent is in exactly ONE
    ongoing project's pipeline, that project is inferred rather than
    demanded up front."""
    group = f"Test Casting {uuid.uuid4().hex[:6]}"
    original = await _use_share_test_config(group)
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    project_id = await _seed_project_with_details(
        f"ShareInferProj {tag}", shoot_dates="7 Jul 2028", budget="Rs 6,000/day",
    )
    talent_id = await _seed_talent(f"ShareInferTalent {tag}", phone="917000700060")
    await _seed_pipeline_row(project_id, talent_id, "ask_to_test")
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f"Share Casting Call with ShareInferTalent {tag}",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
        assert "You are about to SHARE:" in r.reply, r.reply
...
async def test_share_no_project_multiple_pipelines_asks_never_guesses():
    """Same omitted-project case, but the talent is in TWO different
    ongoing projects' pipelines — must ask which one (numbered), never
    silently pick either (Part 6: never guess when ambiguous)."""
    group = f"Test Casting {uuid.uuid4().hex[:6]}"
    original = await _use_share_test_config(group)
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    p1 = await _seed_project_with_details(f"ShareInferA {tag}", shoot_dates="8 Aug 2028", budget="Rs 1/day")
    p2 = await _seed_project_with_details(f"ShareInferB {tag}", shoot_dates="9 Sep 2028", budget="Rs 2/day")
    talent_id = await _seed_talent(f"ShareInferMultiTalent {tag}", phone="917000700061")
    await _seed_pipeline_row(p1, talent_id, "ask_to_test")
    await _seed_pipeline_row(p2, talent_id, "shortlisted")
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f"Share Casting Call with ShareInferMultiTalent {tag}",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
        assert "Which project should I use?" in r.reply, r.reply
        assert f"ShareInferA {tag}" in r.reply and f"ShareInferB {tag}" in r.reply, r.reply
    finally:
...
async def test_share_stage_target_recipients_label_properly_cased():
    ...
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    project_id = await _seed_project(brand_name=f"ShareStageCaseProj {tag}")
    label = (await db.projects.find_one({"id": project_id}))["brand_name"]
    talent_id = await _seed_talent(f"ShareStageCaseTalent {tag}", phone="917000700108")
    await _seed_pipeline_row(project_id, talent_id, "follow_up")
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f"Share Casting Call for {label} to Follow Up",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
...
async def test_share_not_in_pipeline_gate_offers_add_move_share():
    ...
    tag = uuid.uuid4().hex[:6]
    project_id = await _seed_project_with_details(
        f"ShareGateProj {tag}", shoot_dates="10 Oct 2028", budget="Rs 7,000/day",
    )
    talent_id = await _seed_talent(f"ShareGateTalent {tag}", phone="917000700070")
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f"Share Casting Call for ShareGateProj {tag} with ShareGateTalent {tag}",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
...
async def test_share_pipeline_check_missing_from_multiple_projects():
    ...
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    p1 = await _seed_project_with_details(f"GateMultiA {tag}", shoot_dates="1 Jan 2029", budget="Rs 1/day")
    p2 = await _seed_project_with_details(f"GateMultiB {tag}", shoot_dates="2 Feb 2029", budget="Rs 2/day")
    talent_id = await _seed_talent(f"GateMultiTalent {tag}", phone="917000700100")
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f"Share Casting Call for GateMultiA {tag}, GateMultiB {tag} with GateMultiTalent {tag}",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
        assert "PIPELINE CHECK" in r.reply, r.reply
...
async def test_share_pipeline_check_no_stale_context():
    ...
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    p1 = await _seed_project(brand_name=f"GateStaleA {tag}")
    p2 = await _seed_project_with_details(f"GateStaleB {tag}", shoot_dates="7 Jul 2029", budget="Rs 7/day")
    t1 = await _seed_talent(f"GateStaleT1 {tag}", phone="917000700106")
    t2 = await _seed_talent(f"GateStaleT2 {tag}", phone="917000700107")
    ...
    try:
        ...
        r2 = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f"Share Casting Call for GateStaleB {tag} with GateStaleT2 {tag}",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r2.handled, r2.reply
        assert "You are about to SHARE:" in r2.reply, r2.reply
...
async def test_share_never_inherits_session_current_project():
    ...
    share_talent = await _seed_talent(f"StaleSessTalent {tag}", phone="917000700109")
...
async def test_share_bare_quote_custom_message_no_prefix_needed():
    ...
    talent_id = await _seed_talent(f"BareQuoteTalent {tag}", phone="917000700110")
    await _seed_pipeline_row(project_id, talent_id, "ask_to_test")
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
...
async def test_share_ux_prompt_dulux_example_verbatim():
    """SHARE UX & Recipient Preview (2026-09-08) — this task's OWN
    dedicated regression example: a long, multi-line custom message
    containing commas, a period, asterisks, and a hyphen, matched
    almost verbatim to the prompt's own worked example. Every character
    inside the quotes must survive exactly, and the comma inside the
    quote must never be read as a second recipient."""
    group = f"Test Casting {uuid.uuid4().hex[:6]}"
    original = await _use_share_test_config(group)
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    project_id = await _seed_project(brand_name=f"DuluxExampleProj {tag}")
    talent_id = await _seed_talent(f"DuluxExampleTalent {tag}", phone="917000700145")
    await _seed_pipeline_row(project_id, talent_id, "ask_to_test")
    message = (
        "Hi! You've been shortlisted for *DULUX*\n\n"
        "- Please don't change the look in which you gave your audition, "
        "and do not release your dates.\n\n"
        "Shoot date: 6th Sept, Mumbai.\n"
        "Budget: TBD."
    )
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f'Share "{message}" for DuluxExampleProj {tag} with DuluxExampleTalent {tag}',
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
        assert "You are about to SHARE:" in r.reply, r.reply
...
async def test_share_ambiguous_talent_clarification_then_resume():
    ...
    a = await _seed_talent(f"Priya Shah {tag}", phone="917000700140")
...
async def test_share_edit_menu_precedence_over_real_disambiguation():
    ...
    a = await _seed_talent(f"Meera Iyer {tag}", phone="917000700111")
...
async def _await_share_delivery_report(group: str, timeout_sec: float = 3.0) -> Optional[dict]:
    """Polls for the "SHARE COMPLETE" report job the fire-and-forget
    watcher queues back into `group` — a MANUAL/group-routed job, never
    associated with any real talent_id, so it never collides with an
    ordinary talent-lookup query. Filtered by `destination` == `group`
    (the exact WhatsApp group name a "group"-routed job carries) — the
    whole test suite shares one dev DB, and every other SHARE-approval
    test elsewhere ALSO spawns its own watcher, so an unfiltered query
    could otherwise pick up a completely different test's report."""
    ...
    while time.monotonic() < deadline:
        job = await db.whatsapp_jobs.find_one({
            ...
            "destination": group,
            "message_body": {"$regex": "^SHARE COMPLETE"},
        }, sort=[("created_at", -1)])
        if job:
            return job
...
async def _cleanup_share_delivery_reports(group: str) -> None:
    """Deletes ONLY this test's own report job(s)/batch(es) — scoped by
    `destination` == `group`, never a blanket delete that could remove a
    concurrently-running test's own report."""
    jobs = await db.whatsapp_jobs.find(
        {"recipient_kind": "MANUAL", "destination_type": "group", "destination": group},
        {"_id": 0, "batch_id": 1},
    ).to_list(50)
    batch_ids = list({j["batch_id"] for j in jobs})
    if batch_ids:
        await db.whatsapp_batches.delete_many({"id": {"$in": batch_ids}})
        await db.whatsapp_jobs.delete_many({"batch_id": {"$in": batch_ids}})


async def test_share_delivery_report_all_successful():
    """Once the (simulated) real worker marks every queued job "sent",
    the fire-and-forget watcher must report back into the SAME group
    with an accurate success count and the "delivered successfully"
    closing line — never claiming this before the worker actually
    finished."""
    group = f"Test Casting {uuid.uuid4().hex[:6]}"
    original = await _use_share_test_config(group)
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    project_id = await _seed_project_with_details(
        f"DeliverOkProj {tag}", shoot_dates="4 Apr 2029", budget="Rs 4/day",
    )
    talent_id = await _seed_talent(f"DeliverOkTalent {tag}", phone="917000700142")
    await _seed_pipeline_row(project_id, talent_id, "ask_to_test")
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
        ...
        await db.whatsapp_jobs.update_many({"talent_id": talent_id}, {"$set": {"status": "sent"}})

        report = await _await_share_delivery_report(group)
        assert report is not None, "no SHARE COMPLETE report was queued"
        body = report["message_body"]
        assert f"Project: DeliverOkProj {tag}" in body, body
        assert "✓ Successfully sent: 1" in body, body
        assert "✕ Failed: 0" in body, body
        assert "All messages were delivered successfully." in body, body
    finally:
        await _cleanup_share_delivery_reports(group)
        await _cleanup_jobs_for_talents([talent_id])
        await _cleanup(phone, project_ids=[project_id], talent_ids=[talent_id])
        await _restore_share_config(original)


async def test_share_delivery_report_partial_failure():
    """A mix of "sent" and "failed" jobs produces a report naming the
    failed recipient(s) with their real error_message, and the exact
    "were not resent" reassurance for the ones that DID go through —
    never a silent re-send of the successful half."""
    group = f"Test Casting {uuid.uuid4().hex[:6]}"
    original = await _use_share_test_config(group)
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    project_id = await _seed_project_with_details(
        f"DeliverMixProj {tag}", shoot_dates="5 May 2029", budget="Rs 5/day",
    )
    t1 = await _seed_talent(f"DeliverMixT1 {tag}", phone="917000700143")
    t2 = await _seed_talent(f"DeliverMixT2 {tag}", phone="917000700144")
    await _seed_pipeline_row(project_id, t1, "ask_to_test")
    await _seed_pipeline_row(project_id, t2, "ask_to_test")
    try:
        r = await handle_inbound_message(
        ...
        await db.whatsapp_jobs.update_one(
            {"talent_id": t2}, {"$set": {"status": "failed", "error_message": "WhatsApp group unavailable"}},
        )
        ...
        body = report["message_body"]
        assert "✓ Successfully sent: 1" in body, body
        assert "✕ Failed: 1" in body, body
        assert "Failed:" in body, body
        assert f"DeliverMixT2 {tag} — WhatsApp group unavailable" in body, body
        assert "The 1 successful message was not resent." in body, body
    finally:
        await _cleanup_share_delivery_reports(group)
        await _cleanup_jobs_for_talents([t1, t2])
        await _cleanup(phone, project_ids=[project_id], talent_ids=[t1, t2])
...
# ---------------------------------------------------------------------------
# SHARE Ownership/Routing (Production fix, 2026-09-05) — standalone SHARE
# now belongs exclusively to the Talentgram WhatsApp Agent group; the
# Casting Pipeline group rejects/reroutes it instead of executing anything.
# Focused routing tests only (Section 12 of the fix request) — the SHARE
# ENGINE's own correctness (parsing, resolution, cross-product, pipeline
# gate, edit, cancel, ...) is already covered above; these are specifically
# about WHICH GROUP handles the command.
# ---------------------------------------------------------------------------
...
async def test_share_routing_1_casting_pipeline_reroutes_standalone_share():
    ...
    tag = uuid.uuid4().hex[:6]
    project_id = await _seed_project_with_details(
        f"RouteCPProj {tag}", shoot_dates="1 Jan 2030", budget="Rs 1/day",
    )
    talent_id = await _seed_talent(f"RouteCPTalent {tag}", phone="917000700200")
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f"SHARE casting call for RouteCPProj {tag} with RouteCPTalent {tag}",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
        assert r.reply == (
        ...
        assert conv is None, "must not leave a pending confirmation card behind"
    finally:
        await _cleanup(phone, project_ids=[project_id], talent_ids=[talent_id])
        await _restore_config(original)


async def test_share_routing_2_and_3_whatsapp_agent_multi_project_no_old_error():
    """Tests 2/3 — the EXACT reported bug scenario: multi-project SHARE
    in the Talentgram WhatsApp Agent group must invoke the new SHARE
    resolver and resolve BOTH projects correctly — never the old
    "None of the named talent(s) are part of X's pipeline." dead end."""
    group = f"Test WA Agent {uuid.uuid4().hex[:6]}"
    original = await _use_share_test_config(group)
    phone = _phone()
    tag = uuid.uuid4().hex[:6]
    p1 = await _seed_project_with_details(f"RouteHinge {tag}", shoot_dates="2 Feb 2030", budget="Rs 2/day")
    p2 = await _seed_project_with_details(f"RouteLoreal {tag}", shoot_dates="3 Mar 2030", budget="Rs 3/day")
    talent_id = await _seed_talent(f"RouteNikita {tag}", phone="917000700201")
    await _seed_pipeline_row(p1, talent_id, "ask_to_test")
    await _seed_pipeline_row(p2, talent_id, "ask_to_test")
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f"SHARE casting call for RouteHinge {tag},RouteLoreal {tag} with RouteNikita {tag}",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert r.handled, r.reply
        assert "None of the named talent" not in r.reply, r.reply
...
async def test_guided_4_compound_plan_edit_response_shows_step_selector():
    """Guided Step Selector (2026-09-03) — replying "2" to a multi-step
    plan's confirmation NEVER jumps straight into a specific step (that
    was the source of the reported "3" ambiguity — see the module-level
    "Guided Step-Specific Editing" comment). Instead it shows "EDITING
    YOUR PLAN — which step would you like to edit? 1 -> ADD / 2 -> MOVE /
    3 -> SHARE"; only a SUBSEQUENT step-number reply opens that specific
    ...
    try:
        ...
        assert "EDITING YOUR PLAN" in selector.reply, selector.reply
        assert "Which step would you like to edit?" in selector.reply, selector.reply
        assert "1 → ADD" in selector.reply, selector.reply
        assert "2 → MOVE" in selector.reply, selector.reply
        assert "3 → SHARE" in selector.reply, selector.reply
        assert "Or type CANCEL to leave editing." in selector.reply, selector.reply
        # Not yet a specific step's own editor.
        assert "EDITING STEP" not in selector.reply, selector.reply
        ...
        assert f"GuidedPlanTalent {tag}" in step2.reply, step2.reply
        assert "Shortlisted" in step2.reply, step2.reply
        assert "Share" not in step2.reply, step2.reply
        assert "Nothing will execute until you confirm" in step2.reply, step2.reply

        # The literal word "edit" reaches the exact same selector.
        r2 = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=(
                f"Add GuidedPlanTalent {tag} to GuidedPlanProj {tag}, "
        ...
        selector2 = await handle_inbound_message(
            group_name=group, sender_phone=phone, text="edit",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert "Which step would you like to edit?" in selector2.reply, selector2.reply
        assert "1 → ADD" in selector2.reply and "2 → MOVE" in selector2.reply and "3 → SHARE" in selector2.reply

        # CANCEL from inside the selector cancels the whole plan, with
        # the edit-specific wording (never confused with "3").
        cancel = await handle_inbound_message(
            group_name=group, sender_phone=phone, text="CANCEL",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert cancel.reply == "Editing cancelled. Nothing has been executed.", cancel.reply

        # And "3" from the CONFIRMATION card (a fresh command, no editing
        # in progress) is still unambiguously Cancel.
        await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=(
                f"Add GuidedPlanTalent {tag} to GuidedPlanProj {tag}, "
        ...
        cancel3 = await handle_inbound_message(
            group_name=group, sender_phone=phone, text="3",
            sender_name="Raj", sender_is_group_member=True,
        )
...
# ---------------------------------------------------------------------------
# ADD -> MOVE -> SHARE production hardening (2026-09-02/03) — a real
# reported WhatsApp regression: "Add Anusha Sharma, move her to follow up,
# share the casting call with her" (missing project on ADD) fabricated a
# completely unrelated "Move Vikram Sharma to Follow Up in Hinge" step,
# sourced from session.last_talent_id left over by an EARLIER, unrelated
# command. Fixed by making an implicit pronoun on a compound-plan MOVE/
# SHARE step resolve ONLY against what THIS SAME command has already
# touched — never a stale session value from a different command. Also
# covers the Guided Step Selector EDIT UX (replying "2"/"edit" always
# shows "which step?" first, never colliding with the confirmation card's
# own "3 -> Cancel").
# ---------------------------------------------------------------------------
...
async def test_guided_8_reply_3_opens_editing_step_3_share():
    """#8/#10 — on a 3-step plan, from the STEP SELECTOR (never the
...
async def test_guided_8_reply_1_opens_editing_step_1_add_only():
    """#8/#13 — from the step selector, "1" opens "EDITING STEP 1 — ADD"
    specifically; "change ADD to [Talent]" changes ONLY step 1 — steps 2
    and 3 (which refer back to whoever ADD resolves to) correctly follow
    ...
    label = (await db.projects.find_one({"id": project_id}))["brand_name"]
    talent_a = await _seed_talent(f"EditStep1A {tag}")
    talent_b = await _seed_talent(f"EditStep1B {tag}")
    try:
        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f"Add EditStep1A {tag} to {label}, move her to Follow Up, share the casting call with her",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert "You are about to run this plan" in r.reply, r.reply
        ...
        assert "EDITING STEP 1 — ADD" in edit.reply, edit.reply
        assert f"EditStep1A {tag}" in edit.reply, edit.reply

        changed = await handle_inbound_message(
            group_name=group, sender_phone=phone, text=f"change ADD to EditStep1B {tag}",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert "You are about to run this plan" in changed.reply, changed.reply
        numbered_lines = [ln for ln in changed.reply.splitlines() if re.match(r"^\d+\. ", ln)]
        assert len(numbered_lines) == 3, changed.reply
        assert f"EditStep1B {tag}" in numbered_lines[0], changed.reply  # step 1 (Add) — CHANGED
        assert f"EditStep1A {tag}" not in numbered_lines[0], changed.reply
        # Steps 2/3 re-resolve from step 1's own touched_pairs, so they
        # correctly follow the newly-ADDed talent, not the old one.
        assert f"EditStep1B {tag}" in numbered_lines[1], changed.reply
        assert f"EditStep1B {tag}" in numbered_lines[2], changed.reply
        ...
        assert "Completed" in r3.reply, r3.reply
        doc_b = await db.casting_pipeline.find_one({"project_id": project_id, "talent_id": talent_b})
        assert doc_b is not None and doc_b["stage"] == "follow_up", (r3.reply, doc_b)
        doc_a = await db.casting_pipeline.find_one({"project_id": project_id, "talent_id": talent_a})
...
async def test_missing_project_searches_full_catalogue_with_pagination():
    """#1 — the missing-project clarification must search the COMPLETE
    ongoing-project catalogue, never an arbitrary first-N slice — proven
    two ways: (a) the header's own reported total matches the REAL
    database count (this local dev DB already has hundreds of ongoing
    projects, so a stale "first 6" slice would silently show a much
    smaller/wrong number), and (b) a project seeded to sort dead LAST
    alphabetically still resolves correctly by typing its exact name,
    even though it is never on the very first displayed page — proving
    resolution reads the full catalogue, not just whatever page happens
    ...
    talent_id = await _seed_talent(f"PaginateTalent {tag}")
    try:
        import agents.modules.casting_pipeline as cp_module
        real_total = len(await cp_module._fetch_ongoing_projects())

        r = await handle_inbound_message(
            group_name=group, sender_phone=phone,
            text=f"Add PaginateTalent {tag}, move her to follow up, share the casting call with her",
            sender_name="Raj", sender_is_group_member=True,
        )
        assert "ADD needs a project before I can continue." in r.reply, r.reply
        assert f"I found {real_total} active projects." in r.reply, r.reply
        first_page_lines = [ln for ln in r.reply.splitlines() if "→" in ln]
        assert len(first_page_lines) == 8, r.reply  # page size, never a bare "first 6" slice
        assert f"ZZZZZZLastProj {tag}" not in r.reply, r.reply  # sorts dead last, not on page 1
        assert "MORE" in r.reply, r.reply

        more = await handle_inbound_message(
            group_name=group, sender_phone=phone, text="MORE",
            sender_name="Raj", sender_is_group_member=True,
        )
        more_page_lines = [ln for ln in more.reply.splitlines() if "→" in ln]
        assert more_page_lines != first_page_lines, "MORE did not actually advance the page"

        # Reply with the project's own NAME (not a number, not visible on
        # ANY page shown so far) — proves the full catalogue, not just
        # whatever's currently displayed, is what actually resolves it.
        resumed = await handle_inbound_message(
            group_name=group, sender_phone=phone, text=f"ZZZZZZLastProj {tag}",
            sender_name="Raj", sender_is_group_member=True,
        )
