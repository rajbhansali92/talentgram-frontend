...
**Impact**: High
**Evidence**: `applications.py`'s `_reconcile_draft_from_talent()` (called on every `GET /public/apply/{aid}` of a non-submitted draft) sets `should_hydrate_media = True` whenever `len(app_media) != len(talent_media)`, then WHOLESALE REPLACES `app_doc["media"]` with a fresh copy built only from `talent_media` — discarding any items the applicant uploaded directly to their application (not sourced from their talent Library) if the count happens to diverge from the talent's Library count. No storage cleanup for the discarded items either (separate orphan risk).
**Risk**: An applicant who adds extra fresh photos beyond their Library's count can have those uploads silently wiped (DB reference AND file) on their next page load/reload — no warning, no confirmation.
**Recommendation**: Change the hydrate logic from replace to a targeted merge: only add/refresh the subset of `app_media` that's actually sourced from the talent's Library (e.g. by matching id), never touch or remove items that aren't. Needs its own regression pass since `_reconcile_draft_from_talent()` also reconciles several non-media fields (name, location, skills, etc.) that are out of scope for a media-only fix. Found during the Phase 4.4 production-certification audit ([08_DECISION_LOG.md](08_DECISION_LOG.md) D26) but deliberately not fixed there — outside Media Library scope, and this function's other branches weren't covered by that pass's live-testing.
