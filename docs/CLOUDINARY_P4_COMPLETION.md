...
**Reviewed & corrected (P4 merge check):** Talentgram's supported browser matrix is `frontend/package.json` browserslist `"production"` = `>0.2%, not dead, not op_mini all` — Chrome / Edge / Firefox (incl. ESR) / Safari, desktop + mobile, current **and older** releases. `LazyVideoPlayer` / `HlsVideo` assign the URL straight to `<video>.src` with **no `MediaCapabilities` probe and no codec fallback**, so the served `url` must decode natively there.

`core.video_needs_compat_delivery(fmt, codec)` → `True` (→ compat delivery) only for:
- `codec ∈ {hevc, h265, hvc1, hev1, prores, dnxhd, mjpeg, wmv1/2/3, vc1, mpeg4, msmpeg4*, rv30/40, theora, flv1, cinepak, svq3}`, or
- `format ∈ {avi, wmv, flv, mkv, mpeg, mpg, ogv, rm, rmvb, asf, vob, divx}`.

**HEVC / H.265 is a compat exception** (corrected — it was previously assumed safe). Rationale: Safari-only for years; Chrome needs 107+ **and** a platform hardware HEVC decoder (fails on many desktops, esp. Linux); Firefox only 134+ (Jan 2025) with an OS decoder — Firefox ESR / older = no decode. Common as an iPhone "High Efficiency" `.mov` intro-video upload; **admin audition takes arrive via WhatsApp already re-encoded to H.264 and are unaffected**. Precedence: codec is authoritative, checked before container.

Served as the uploaded original (no transform): `mp4/mov/webm` with `h264/avc1`; `webm` with `vp8/vp9`; `mp4` with `av1` (rare as an upload; revisit on volume). **Unknown format AND unknown codec → serve the original** (conservative cost policy preserved — unknown ≠ transform). Never inferred from the file extension alone; `fmt`/`codec` come from Cloudinary's response.
...
### Compatibility matrix (as tested — `test_p4_no_eager_transformations.py::COMPAT_MATRIX`)

| format | codec | decision | reason |
|---|---|---|---|
| mp4 / mov | h264 / avc1 | **serve original** | universal |
| webm | vp9 / vp8 / h264 | **serve original** | Chrome/Edge/FF always; Safari 14.1+ |
| mp4 | av1 | **serve original** | Chrome/FF/Edge; Safari 17+; rare as an upload |
| mp4 / mov | **hevc / h265 / hvc1 / hev1** | **compat delivery (f_mp4)** | Firefox ESR / non-HW Chrome can't decode HEVC |
| avi | (any) | **compat delivery** | AVI container unsupported by `<video>` |
| wmv | wmv3 / (any) | **compat delivery** | Windows Media — unsupported |
| mkv | h264 | **compat delivery** | MKV container: Safari/Firefox won't |
| mpeg / mpg | (any) | **compat delivery** | MPEG-PS container unsupported |
| flv | flv1 | **compat delivery** | Flash video — unsupported |
| mp4 | prores / mjpeg | **compat delivery** | pro / webcam codec — no browser |
| (unknown) | (unknown) | **serve original** | conservative — unknown ≠ transform |
