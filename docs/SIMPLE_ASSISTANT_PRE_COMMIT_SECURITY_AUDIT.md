...
## Execution kill-switch — `SA_EXECUTION_ENABLED` (later update, same workstream)

A subsequent read-only-activation review surfaced a genuine gap this audit's original §7 did not call out explicitly enough: **`SIMPLE_ASSISTANT_ENABLED` alone exposes both the read-only surface and the confirm/upload endpoints together** — there was no independent flag that kept mutation/send/upload execution off while the master flag was on for read-only verification. This section documents the fix.

### What each flag now controls

- **`SIMPLE_ASSISTANT_ENABLED`** (default OFF) — exposes the `/api/simple-assistant/*` router at all. With this off, every route (including `/overview` and `/command`) 404s, regardless of every other flag below.
- **`SA_EXECUTION_ENABLED`** (default OFF, new) — independently gates the five functions that are the *only* paths in this module able to call a canonical mutating, WhatsApp-sending, or Cloudinary-uploading function: `execute.confirm_and_execute`, `comm_execute.confirm_and_send`, `submission_execute.confirm_and_attach`, `submission_execute.confirm_and_ingest`, and `ai_response_execute.confirm_and_send_ai`. The check is the first substantive thing each function does (right after verifying the HMAC signature and the plan `kind`) — so it applies no matter which route, or any future caller, reaches the function. `/overview` and `/command` never call any of these five functions and are completely unaffected by this flag. **`SIMPLE_ASSISTANT_ENABLED=true` + `SA_EXECUTION_ENABLED` left off is now a genuine, technically-enforced read-only production configuration** — not merely a testing-discipline convention.
- **`SA_AI_RESPONSE_ENABLED`** (default OFF) — unchanged, still gates only whether a draft is *generated* in the first place (in `ai_response.py`, called from `commands.py`'s preview-time AI-draft branch). Completely independent of `SA_EXECUTION_ENABLED`, which only gates the later, separate *approve & send* step.
- **`SA_INBOUND_CAPTURE_ENABLED`** (default OFF) — unchanged, gates the separate worker-webhook inbound-capture hook in `routers/agents_whatsapp.py`, outside the Simple Assistant router entirely. Untouched by, and irrelevant to, `SA_EXECUTION_ENABLED`.
- **`SA_INBOUND_INTELLIGENCE_ENABLED`** / **`SA_CONVERSATION_CONTEXT_ENABLED`** — unchanged, both still independent of the new flag.

### Enforcement

`is_execution_enabled()` (new, in `simple_assistant/__init__.py`, alongside `is_enabled()`) is called as the first check in each of the five functions above — before any structural validation, expiry check, idempotency check, live re-resolution, or the mutating/sending/uploading call itself. For the upload path specifically (`confirm_and_ingest`), the check runs **before the multipart file body is ever read into memory** — proven by a dedicated test using an upload stand-in that tracks whether `.read()` was called (`tests/test_simple_assistant_ingest.py::test_execution_disabled_blocks_confirm_upload_before_reading_the_file`). None of the five existing safeguards these functions already had (HMAC verification, the `expires_at` TTL check added earlier in this same document, stale-state/live-re-resolution checks, idempotency via `audit.find_completed`, or `validate_draft()`) were touched, reordered, or weakened — the new check is purely additive, sitting in front of all of them. A block returns a `state: "blocked"` envelope with a single shared message (`EXECUTION_DISABLED_MESSAGE`, also new in `__init__.py`) and is deliberately **not** recorded to the audit log, so a plan that was valid when execution was off remains valid and confirmable once execution is turned on — it is not consumed or invalidated by the block. Every one of the five new tests proves this explicitly: block with the flag off, then flip the flag on and confirm the *same* signed context succeeds.

`POST /command/comm-status` (`comm_execute.comm_status`) was inspected and deliberately left ungated: it only reads back the status of jobs that were already queued by a *previous, successful* `confirm_and_send`/`confirm_and_send_ai` call — it creates nothing itself, so it cannot be a bypass. `whatsapp_send.preview_destination` was also inspected and found to be dead code (defined, never called anywhere in the module) — noted for completeness, not gated, not removed (out of scope for this task).

### Frontend

No frontend code change was made. The four confirmable card components in `AssistantConsole.jsx` (`PlanCard`, `CommPreviewCard`, `SubmissionPreviewCard`, `IntelligenceCard`) were inspected and already handle a `state: "blocked"` response correctly for every action type: the response's `message` is rendered in a dedicated, clearly-marked element (`data-testid="assistant-{exec,comm,sub,intel}-blocked"`), and the "Confirm" button is *not* re-rendered afterward (the two branches are mutually exclusive in each component's render logic) — so a user who clicks Confirm while execution is disabled immediately sees the exact reason nothing happened, with no way to retry-loop into a false impression that something is pending. This was pre-existing infrastructure, not added for this change; it happened to already be correct for a new kind of `blocked` response because the UI branches on `state`, not on why the state is what it is.

### Read-only behavior preserved

`is_execution_enabled()` is never consulted by `/overview` (`service.build_overview`) or `/command`'s preview path (`commands.py`, which only ever touches the database through the structural read-only proxy `readonly_db.RDB`) — both remain fully available with `SIMPLE_ASSISTANT_ENABLED=true` and `SA_EXECUTION_ENABLED` left off. Every one of the five new execution-gate tests explicitly confirms the corresponding preview step still succeeds before the (now-blocked) confirm attempt.
...
| Execution flag (`SA_EXECUTION_ENABLED`, added later this workstream — see "Execution kill-switch" above) defaults disabled | ✅ `is_execution_enabled()` defaults `"false"`; independently gates the five mutating/sending/uploading functions, unaffected by every other flag |
...
| Railway and Vercel environment expectations documented | `GEMINI_API_KEY` (or `ANTHROPIC_API_KEY` if `SA_AI_PROVIDER=anthropic`) must be set on the `talentgram-railway` service for the AI-response feature to function once enabled; `SIMPLE_ASSISTANT_ENABLED`/`SA_EXECUTION_ENABLED`/`SA_INBOUND_CAPTURE_ENABLED`/`SA_INBOUND_INTELLIGENCE_ENABLED`/`SA_AI_RESPONSE_ENABLED`/`SA_CONVERSATION_CONTEXT_ENABLED` are all optional, all default OFF; `NEXT_PUBLIC_SIMPLE_ASSISTANT_ENABLED` must be set on the Vercel frontend project to reveal the nav entry (the route itself additionally self-redirects when this is unset, §1.6). None of these need to be set for the code to be safely committed and deployed in its current, fully-dormant state. **Do not claim the system is read-only in any environment unless `SA_EXECUTION_ENABLED` is confirmed off there** — `SIMPLE_ASSISTANT_ENABLED` alone is not a read-only guarantee. No feature flag in this module should be enabled without reviewing its specific side effects first. |
