...
## D26: Media Deletion Is Reference-Aware Everywhere, Not Just In The Media Library

**Decision**: Every code path that can physically destroy a Cloudinary/Cloudflare Stream asset — not only the Media Library's own delete endpoints — must first confirm no other document (`talents`/`submissions`/`applications`) still references that same asset. `sync_media_to_global_talent()` (and the equivalent `/apply` hydrate logic) must copy every field a source media item has, not a hand-picked subset, so a copy can always be correctly identified and reference-checked later regardless of which storage provider produced it.

**Rationale**: The Media Library Manager (Phase 4.3, commit `7df26fb`) built a talent-owned delete endpoint and, during its own live testing, discovered that `talents.media[]`/`submissions.media[]`/`applications.media[]` share physical storage objects **by value** (`build_prefill_media()`/`sync_media_to_global_talent()` copy `public_id`/`stream_uid`, never re-upload) — so an unconditional delete on one collection's copy could destroy a still-needed asset on another. Phase 4.3 fixed this for the Library's own two delete routes only. The Phase 4.4 production-certification pass (this entry) traced the full "Future Submission → Delete" lifecycle per the audit checklist and found the identical unconditional-destroy pattern in FIVE more places that were never covered: (1) the submission single-slot video-replace path, (2) the talent's own `DELETE /public/submissions/{sid}/media/{mid}` (confirmed live: removing a "reused" Library photo from a brand-new submission destroyed the talent's own Library original), (3) the Cloudinary webhook's intro-video-replace cleanup, (4) the application media-delete endpoint, (5) `sync_media_to_global_talent()`'s own Library-side video single-slot replace. Separately, `sync_media_to_global_talent()`'s mirror used a fixed field whitelist that silently dropped `provider`/`stream_uid` — meaning even where the reference-aware guard *did* exist (Phase 4.3), it could never actually protect a mirrored Cloudflare Stream video, because the copy had lost the one field (`stream_uid`) needed to identify it as a Stream asset at all.

**Implementation**:
- `core.is_media_asset_referenced(public_id, stream_uid)` — unchanged from Phase 4.3, now backed by sparse indexes on `media.public_id`/`media.stream_uid` across all three collections (added this pass; was previously an unindexed collection scan on every delete).
- New: `core.safe_cleanup_media_storage(media, scope, parent_id, operation_id=None)` — the single function that pairs the reference check with the actual multi-provider destroy (`cleanup_media_storage()`). `delete_talent_media_item()` was refactored to call it instead of duplicating the check inline. All five newly-discovered call sites (above) were switched from the raw `cleanup_media_storage()` to this wrapper.
- `sync_media_to_global_talent()`'s mirror-building changed from a fixed field whitelist to a deny-list copy: every field on the source item is carried over except an explicit list of ownership/location/processing-state fields (`id`, `scope`, `submission_id`, `origin`, `status`, `client_visible`, etc. — see `core.py` for the current list). This is provider-agnostic by construction: a future storage provider's own identifying field is preserved automatically, with no code change to this function. `applications.py`'s independent hydrate-media logic received the equivalent fix (own deny-list, since it has a different category-remap and is a separate, not-yet-consolidated implementation — see [04_MEDIA_RULES.md](04_MEDIA_RULES.md)'s "Canonical prefill" section).
- `sync_media_to_global_talent()` also now no-ops when the source item has no `url` yet (still processing), closing a race where finalizing a submission before an async video transcode completes could freeze a permanently-broken item into the Library.
- `cloudinary_admin.py`'s `run_storage_cleanup` (admin health-cleanup tool) had its own independent reference set extended to include `db.applications` (previously only checked `talents`/`submissions`/`asset_metadata`/`feedback` — could have wrongly treated an application-hydrated asset as orphaned and destroyed it).
- All of the above were live-verified against the real local Cloudinary account (not mocks): confirmed via the Cloudinary Admin API (not the CDN, which serves stale cached 200s after an origin delete) that a shared asset survives when another collection still references it, and that a genuinely unreferenced asset is still actually destroyed (no over-protection regression).

**Not fixed in this pass** (flagged, see [07_OPEN_ISSUES.md](07_OPEN_ISSUES.md)): Cloudflare Stream video replacement has no cleanup call at all for the old asset (a pure orphan-leak, not a data-loss risk — lower urgency, but real cost at scale); `applications.py`'s `_reconcile_draft_from_talent()` can wholesale-replace `app_doc.media` (discarding an applicant's own fresh uploads) under an unrelated length-mismatch heuristic — a real bug, but outside Media Library scope and not something to rewrite under this audit's time budget without dedicated regression coverage of that function's other (non-media) reconciliation branches.

**Established by**: This session (2026-07-28), Phase 4 item 4 "Production Hardening & Release Certification", following Phase 4.3 (commit `7df26fb`).

---

## D27: `/apply` Draft-Media Reconciliation Is Additive-Only, Matched By Id

**Decision**: `_reconcile_draft_from_talent()`'s media logic never removes or replaces an existing `application.media[]` item, for any reason. It only ever ADDS a talent Library item that the application doesn't already have — determined by comparing ids, not counts or category sets. No new schema field was introduced to mark "hydrated vs. applicant-owned" media.

**Rationale**: D26 flagged this as a release blocker: `_reconcile_draft_from_talent()`, which runs on every `GET /public/apply/{aid}` of a non-submitted draft — i.e., every ordinary page refresh, no user action required — used `len(app_media) != len(talent_media)` as a proxy for "the talent's Library changed since we last hydrated." That heuristic cannot distinguish "the Library changed" from "the applicant uploaded more of their own photos than exist in their Library" (the ordinary, common state for any returning talent applying again) — so on any count mismatch it wholesale-replaced `app_doc.media`, silently discarding the applicant's own uploads with no storage cleanup. Root cause: `application.media[]` items carried no marker distinguishing "copied from the talent's Library" from "the applicant's own upload" — unlike `submission.media[]`, which got an `origin` field in Phase 4.1 for exactly this reason.

An id-based additive merge solves the same problem without a schema change: `_reconcile_draft_from_talent()` (and its predecessor, before this fix) always creates a hydrated item with the SAME `id` as its source `talent.media` item — so "is this Library item already reflected in the draft" is answerable by simple set membership on existing ids, with no marker needed. This is also backward-compatible for free: a draft hydrated by the pre-fix code already has items sharing ids with `talent_media`, so the new logic recognizes them as "already present" on its very first post-fix run — no migration, no duplication, no data loss for in-flight drafts.

Fixing this surfaced a second, independent bug: `upload_application_media` unconditionally mirrors every fresh application upload into `talents.media` via `sync_media_to_global_talent()` (a brand new id each time, `source_application_media_id` pointing back at the original). A pure "add any talent item not yet in app_media by id" merge would see that mirror as a genuinely new Library item on the next reconciliation and copy it straight back into the application — duplicating the applicant's own photo under a second id, discovered via live testing (not by reading the code a second time) when a test scenario showed 6 items where 4 were expected.

**Implementation**: `backend/routers/applications.py`, `_reconcile_draft_from_talent()`'s media block (previously "Media sync logic," now "Media hydration — ADDITIVE ONLY"):
- `existing_ids = {m["id"] for m in app_media}` computed once.
- For each `talent_media` item: skip if its id is already in `existing_ids`; skip if its `source_application_media_id` is already in `existing_ids` (the mirror-loop guard above); otherwise build the same provider-agnostic field-preserving copy from D26 and append it.
- `should_hydrate_media`, the old count/category-set comparison, and the `force_refresh`-drives-media-replace branch are removed entirely — no longer needed, since the merge is now unconditionally safe to attempt on every call (it's a no-op when there's nothing new to add).
- Zero changes to `upload_application_media`, `delete_application_media`, `finalize_application`, or any of the non-media reconciliation fields (name/location/skills/etc.) in the same function — those were out of scope and untouched.

**Live verification**: an existing talent's application survives two full refresh cycles with the applicant's own uploads AND the hydrated Library items both intact, exact count, no duplication; a new applicant's uploads survive three refresh/resume cycles through to finalize; a legacy pre-fix draft (id-shared with `talent_media`, no marker) is not duplicated on its first post-fix load. Full Phase 4.4 Media Library regression + security suites re-run clean (this fix touches only `applications.py`, no shared helper changed).

**Established by**: This session (2026-07-28), "Release Blocker: `/apply` Draft Reconciliation Data-Loss Audit & Fix", following D26.
