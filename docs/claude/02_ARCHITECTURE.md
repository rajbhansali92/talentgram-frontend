...
- **Submission Experience Model + Upload Activity Model**: The readiness checklist, section-status badges, Submit CTA, and the Upload Activity Panel are all pure renderers of two presentation-model hooks (`useSubmissionExperienceModel`, `useUploadActivityModel`), which in turn aggregate the Requirement/Operational/Readiness engines (`lib/requirementEngine.js`, `lib/readinessStatus.js`). See [09_PRESENTATION_MODELS.md](09_PRESENTATION_MODELS.md) for the full layering and dependency-direction rule.
